...
with open(csvpath) as csvfile, open(output_folder, "w") as output:
    ...
    row_count=1                                                         #to count rows excluding the header row
    ...
    first_Row=next(csvreader)                                           #so we compare row 3 to row 2 first
    prev_net=int(first_Row[1])                                          #setting cell B2 as the previous value to calc change in profit
    total_sum=prev_net                                                  #start total_sum calc from cell B2
